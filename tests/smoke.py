...
assert greeting.replace("Hey", index=0, length=5) is False
assert str(greeting) == "Hello, world!"

assert greeting.replace("?", index=12, length=1) is False
assert str(greeting) == "Hello, world!"

assert greeting.replace(" there", index=5, length=7) is True
